...
export default () => {
  ...
  const { showMajor } = route.params;
  const [editShowMajor, setEditShowMajor] = useState(showMajor);
  const navigation = useNavigation();
  const handleUpdateShowMajor = () => {
    const newShowMajorValue = editShowMajor === 'No one' ? false : true;
    showMajorAPI({showMajor: newShowMajorValue});
  ...
  return (
    ...
    <View style={styles.container}>
      <Text style={styles.title}>Change your settings for major</Text>
      ...
      <Dropdown
        ...
        labelField="label"
        data={whoCanSeeData}
        onChange={item => setEditShowMajor(item.value)}
        value={showMajor}
      ...
      <TouchableOpacity
        ...
        onPress={saveProfile}
      >
        <Text style={styles.saveText}>Save Settings For Major</Text>
      </TouchableOpacity>
    </View>
  );
}
