...
export default () => {
  ...
  const { showCourse } = route.params;
  const [editShowCourse, setEditShowCourse] = useState(showCourse);
  const navigation = useNavigation();
  const handleUpdateShowCourse = () => {
    const newShowCourseValue = editShowCourse === 'No one' ? false : true;
    showCourseAPI({showCourse: newShowCourseValue});
  ...
  return (
    ...
    <View style={styles.container}>
      <Text style={styles.title}>Change your settings for course</Text>
      ...
      <Dropdown
        ...
        labelField="label"
        data={whoCanSeeData}
        onChange={item => setEditShowCourse(item.value)}
        value={showCourse}
      ...
      <TouchableOpacity
        ...
        onPress={saveProfile}
      >
        <Text style={styles.saveText}>Save Settings For Course</Text>
      </TouchableOpacity>
    </View>
  );
}
