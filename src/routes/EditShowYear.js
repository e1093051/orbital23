...
export default () => {
  ...
  const [editShowYear, setEditShowYear] = useState(showYear);
  const navigation = useNavigation();
  const handleUpdateShowYear = () => {
    const newShowYearValue = editShowYear === 'No one' ? false : true;
    showYearAPI({showYear: newShowYearValue});
  ...
  return (
    ...
    <View style={styles.container}>
      <Text style={styles.title}>Change your settings for year</Text>
      ...
      <Dropdown
        ...
        labelField="label"
        data={whoCanSeeData}
        onChange={item => setEditShowYear(item.value)}
        value={showYear}
      ...
      <TouchableOpacity
        ...
        onPress={saveProfile}
      >
        <Text style={styles.saveText}>Save Settings For Year</Text>
      </TouchableOpacity>
    </View>
  );
}
